...
<html lang="en">
  <head>
    <title>Fresh Station Supermarket</title>
    <meta charset="utf-8">
    ...
    <link href="<?php echo base_url() ?>assets/assets_dashboard/css/vendor.css" rel="stylesheet">
    <link href="<?php echo base_url() ?>assets/assets_dashboard/style.css" rel="stylesheet">
    ...
    <link href="<?php echo base_url() ?>assets/assets_dashboard/css/cart.css" rel="stylesheet">
  ...
  <body>
    ...
    <div class="offcanvas-body">
        <div class="order-md-last" id="offcanvasCartBody">
            <!-- Isi cart akan diupdate di sini oleh AJAX -->
            <h4 class="d-flex justify-content-between align-items-center mb-3">
                <span class="text-primary">Your cart</span>
                <span class="badge bg-primary rounded-pill">
                    <?php echo !empty(session('cartItems')) ? count(session('cartItems')) : 0 ?>
                </span>
            </h4>
            <?php if (!empty(session('cartItems'))) : ?>
                <ul class="list-group mb-3">
                    <?php foreach (session('cartItems') as $item) : ?>
                        <li class="list-group-item d-flex justify-content-between lh-sm">
                            <div>
                                <h6 class="my-0"><?= $item['nama_produk'] ?></h6>
                                <small class="text-body-secondary">Quantity: <?= $item['quantity'] ?></small>
                            </div>
                            <span class="text-body-secondary">Rp<?= number_format($item['harga_jual'] * $item['quantity'], 2) ?></span>
                        </li>
                    <?php endforeach ?>
                    <li class="list-group-item d-flex justify-content-between">
                        <span>Total (IDR)</span>
                        <strong>Rp<?= number_format(session('cartTotal') ?? 0, 2) ?></strong>
                    </li>
                </ul>
    ...
    <div class="offcanvas offcanvas-start" tabindex="-1" id="offcanvasNavbar">
      ...
      <div class="offcanvas-body">
        ...
        $categories = $db->table('type')
                        ->select('kode_type, nama_type, img')
                        ->get()
                        ->getResultArray();
        ?>
        <ul class="navbar-nav justify-content-end menu-list list-unstyled d-flex gap-md-3 mb-0">
          ...
          <li class="nav-item border-dashed">
            <a href="<?= base_url('products?type=' . $category['kode_type']) ?>" class="nav-link d-flex align-items-center gap-3 text-dark p-2">
              <?php if (!empty($category['img'])): ?>
                <img src="<?= base_url('assets/uploads/' . $category['img']) ?>" alt="<?= $category['nama_type'] ?>" style="width: 32px; height: 32px; object-fit:cover; border-radius:50%;">
              <?php else: ?>
                <svg width="24" height="24" viewBox="0 0 24 24"><use xlink:href="#category"></use></svg>
              <?php endif; ?>
              <span><?= $category['nama_type'] ?></span>
            </a>
          </li>
          <?php endforeach; ?>
        </ul>
    ...
    <header class="position-relative bg-white">
      <div class="container-fluid px-4"> <!-- Added px-4 for better spacing -->
        <div class="row py-3 border-bottom">
          ...
          <div class="col-sm-4 col-lg-2 text-center text-sm-start">
            <div class="d-flex align-items-center justify-content-center justify-content-sm-start gap-3">
              <a href="<?= base_url('/') ?>">
                <img src="<?= base_url('assets/assets_dashboard/images/logo.svg') ?>" alt="logo" class="img-fluid" width="80" height="30">
              </a>
          ...
          <div class="col-sm-6 offset-sm-2 offset-md-0 col-lg-4 d-flex align-items-center">
            <div class="search-bar row bg-light p-2 rounded-4 w-100">
              <div class="col-11">
                <form id="search-form" action="<?= base_url('products/search') ?>" method="get" class="text-center">
                  <input type="text" name="keyword" class="form-control border-0 bg-transparent" placeholder="Cari produk..." value="<?= isset($_GET['keyword']) ? $_GET['keyword'] : '' ?>">
                </form>
              </div>
              <div class="col-1">
                <button type="submit" form="search-form" class="btn btn-link p-0 text-dark">
          ...
          <div class="col-lg-4 d-flex align-items-center">
            <ul class="navbar-nav list-unstyled d-flex flex-row gap-2 justify-content-center flex-wrap align-items-center mb-0 fw-bold text-uppercase text-dark">
              <li class="nav-item">
                <a href="<?= base_url('/') ?>" class="nav-link px-2">Beranda</a>
              </li>
              <li class="nav-item">
                <a href="<?= base_url('/products') ?>" class="nav-link px-2">Belanja</a>
              </li>
              <li class="nav-item dropdown">
                <a class="nav-link dropdown-toggle" role="button" id="pages" data-bs-toggle="dropdown" aria-expanded="false">Menu</a>
                <ul class="dropdown-menu border-0 p-3 rounded-0 shadow" aria-labelledby="pages">
                  <li><a href="<?= base_url('pages/about') ?>" class="dropdown-item">Tentang Fresh Station</a></li>
                  <li><a href="<?= base_url('products') ?>" class="dropdown-item">Daftar Produk</a></li>
                  <li><a href="<?= base_url('cart') ?>" class="dropdown-item">Keranjang Belanja</a></li>
                  <li><a href="<?= base_url('pages/faq') ?>" class="dropdown-item">FAQ / Pusat Bantuan</a></li>
                  <li><a href="<?= base_url('pages/contact') ?>" class="dropdown-item">Kontak Kami</a></li>
                </ul>
              </li>
            </ul>
          </div>
          ...
          <div class="col-sm-8 col-lg-2 d-flex gap-5 align-items-center justify-content-center justify-content-sm-end">
            <ul class="d-flex justify-content-end list-unstyled m-0 align-items-center">
              <li class="nav-item dropdown">
                <?php if (logged_in()) : ?>
                    <a href="#" class="p-2 mx-1 dropdown-toggle header-icon" data-bs-toggle="dropdown" aria-expanded="false">
                        <svg width="30" height="30"><use xlink:href="#user"></use></svg>
                    </a>
                    <ul class="dropdown-menu dropdown-menu-end border-0 shadow">
                        <li><span class="dropdown-item-text">Hello, <?= user()->username ?></span></li>
                        <li><hr class="dropdown-divider"></li>
                        <li><a class="dropdown-item" href="<?= base_url('profile') ?>">Profile saya</a></li>
                        <li><a class="dropdown-item" href="<?= base_url('customer/orders') ?>">pesanan saya</a></li>
                        <li><a class="dropdown-item" href="<?= base_url('logout') ?>">Logout</a></li>
                    </ul>
                <?php else : ?>
                    <a href="<?= base_url('login') ?>" class="p-2 mx-1 header-icon">
                        <svg width="30" height="30"><use xlink:href="#user"></use></svg>
                    </a>
                <?php endif; ?>
              </li>
              ...
              <li class="d-flex align-items-center">
                <a href="#" class="p-2 mx-1 position-relative header-icon" data-bs-toggle="offcanvas" data-bs-target="#offcanvasCart" aria-controls="offcanvasCart">
                  <svg width="30" height="30"><use xlink:href="#shopping-bag"></use></svg>
                  <?php if (session()->has('cartItems') && !empty(session('cartItems'))) : ?>
    ...
    <script>
      // Fungsi untuk reload isi keranjang di offcanvas
      function reloadOffcanvasCart() {
        ...
        .then(html => {
          // Update langsung isi cart
          document.getElementById('offcanvasCartBody').innerHTML = html;
        });
      }

      // Contoh pemanggilan: panggil reloadOffcanvasCart() setelah addToCart sukses
      // addToCart(...).then(() => reloadOffcanvasCart());
    </script>

    <script>
      // Function to update cart badge
      function updateCartBadge(count) {
        const cartLink = document.querySelector('[data-bs-target="#offcanvasCart"]');
    ...
    <?php if (logged_in()) : ?>
      <div class="live-chat-btn" id="liveChatBtn">
        ...
        <span class="unread-badge chat-modal-hidden" style="line-height: 20px; padding: 0 6px; text-align: center;"></span>
      </div>

      <!-- Chat Modal -->
      <div class="chat-modal chat-modal-hidden" id="chatModal">
        <div class="chat-header">
          <div>
            <h5 class="mb-0">Live Chat</h5>
            <small>Admin akan membalas secepatnya</small>
          </div>
        ...
        <div class="chat-input-area">
          <input type="text" class="form-control" id="chatInput" placeholder="Ketik pesan Anda...">
    ...
    <?php else : ?>
      ...
      <div class="chat-modal" id="chatModal">
        <div class="chat-header">
          <div>
            <h5 class="mb-0">Live Chat</h5>
            <small>Login untuk menggunakan live chat</small>
          </div>
          <button type="button" class="btn-close btn-close-white" id="closeChatBtn"></button>
        </div>
        <div class="chat-messages text-center p-4">
          <div class="mb-4">
            <i class="fas fa-lock fa-3x text-muted"></i>
          </div>
          <h5>Login Diperlukan</h5>
          <p class="text-muted mb-4">Silakan login terlebih dahulu untuk menggunakan fitur live chat</p>
          <button class="btn btn-primary" id="chatLoginBtn">Login Sekarang</button>
        </div>
      </div>
      ...
      <script>
        document.addEventListener('DOMContentLoaded', function() {
          ...
          chatLoginBtn.addEventListener('click', () => {
            chatModal.style.display = 'none'; // Close chat modal first
            window.location.href = '<?= base_url('login') ?>?redirect=<?= base_url('/') ?>'; // Add redirect parameter
          });
        });
      </script>
    <?php endif; ?>
